...
export function getPossibleDoorPositions(
  ...
  const hasOverlap = proposed => rooms.some(r => _roomsOverlap(proposed, r));
  ...
  for (const existingRoom of rooms) {
    for (const sourceDoor of existingRoom.potentialDoors) {
      ...
      for (const targetDoor of targetRoom.potentialDoors) {
        if (!_doorsAreCompatible(sourceDoor.side, targetDoor.side)) {
          continue;
        }
...
function _doorsAreCompatible(sourceSide, targetSide) {
  const opposites = {
    left: 'right',
    right: 'left',
    top: 'bottom',
